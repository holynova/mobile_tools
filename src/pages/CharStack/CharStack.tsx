...
import { Button, Input, Slider } from "antd-mobile";
import "./CharStack.scss";
import DebugPanel from "../../common/components/DebugPanel";
import Char from "./Char";
import FontPicker from "./components/FontPicker";
import { useFontPicker } from "./hooks/useFontPicker";
import { getSteps } from "../Rainbow/rainbow";
// import  {log} from ''
interface Props {}
const str1 = `
为什么物理学中先假设理论是对称的，再让其破缺？
为什么不能是「理论本身就是不对称的」？从而也就不存在「对称性破缺」了。 「建立理论的时候，先设定对称性再破坏`.repeat(
  10
);

const str2 = "我".repeat(100);
...
const CharStackPage: React.FC<Props> = (props) => {
  // const [loading, setLoading] = useState(false)

  const { show } = useFontPicker();
  const [charList, setCharList] = useState(
    standard1.substring(0, 100).split("")
  );
  ...
  const renderCharPart = useCallback(() => {
    const colors = getSteps(0, 360, charList.length, (x) => {
      return `hsl(${x},100%,50%)`;
    });
    return (
      <div className="canvas">
        {charList.map((char, index) => {
          return (
            <Char key={index} style={{ ...style, color: colors[index] }}>
              {char}
            </Char>
          );
        })}
  ...
  return (
    <div className="CharStackPage">
      <h3>CharStackPage</h3>
      <h4>{charList.length}</h4>
      <Button onClick={show}>font</Button>

      <DebugPanel data={{ len: charList.length, opacity, font }}></DebugPanel>
      <Slider
        value={opacity}
        onAfterChange={(val) => setOpacity(val)}
        min={1}
        max={100}
      ></Slider>
      {renderCharPart()}
